...
* **sample_profile** / **sample_profile_primary_sample** / **sample_profile_aliquot_sample** / **sample_profile_sample_column** (2026-09-25): ✅ **COMPLETE**. Migrated all 4 DB2 profile tables (10/15/29/4-of-40 rows) using [sample_profile_manifest.yaml](file:///Users/muilu/git/others/biobank-solution/sample-service-migration/config/manifests/sample_profile_manifest.yaml), [sample_profile_primary_sample_manifest.yaml](file:///Users/muilu/git/others/biobank-solution/sample-service-migration/config/manifests/sample_profile_primary_sample_manifest.yaml), [sample_profile_aliquot_sample_manifest.yaml](file:///Users/muilu/git/others/biobank-solution/sample-service-migration/config/manifests/sample_profile_aliquot_sample_manifest.yaml) and [sample_profile_sample_column_manifest.yaml](file:///Users/muilu/git/others/biobank-solution/sample-service-migration/config/manifests/sample_profile_sample_column_manifest.yaml), extracted by [export_sample_profiles.py](file:///Users/muilu/git/others/biobank-solution/sample-service-migration/scripts/export_sample_profiles.py). `sample_profile_primary_sample` has no literal business-key column, so the aliquot table's parent FK needed one extra read-only bridging step, [enrich_sample_profile_aliquots.py](file:///Users/muilu/git/others/biobank-solution/sample-service-migration/scripts/enrich_sample_profile_aliquots.py) (run between the primary-sample and aliquot-sample load steps) — see [sample-profile-migration-plan.md](file:///Users/muilu/git/others/biobank-solution/sample-service-migration/docs/sample-profile-migration-plan.md) §4 for why the generic FK resolver alone can't do this hop. 36 of the 40 `sample_profile_sample_column` rows (9 of 10 distinct legacy field names) and several `BIOBANK_SAMPLE_PROFILE`/`PROFILE_SAMPLE_AMOUNT` columns carrying real (non-default) legacy data (kit-creation/lab-request toggles, lab package code, product codes) have no destination in the current schema and are intentionally dropped — documented, not silently lost; see the plan doc §2-3 for the full list and the 2026-09-25 decision to drop-and-document rather than extend the schema.
...
- **Boolean column support (2026-09-25, user-approved)**: `BatchLoader.convertToType` previously had no branch for boolean columns (no manifest had ever targeted one) and silently bound `'true'`/`'false'` strings, which Postgres rejects against a real `boolean` column. Added a `type.contains("BOOL")` branch parsing into a real `Boolean`. Use `type: "BOOLEAN"` in the manifest (or omit it and rely on auto-discovery) with a `transformScript` normalizing the source value to `'true'`/`'false'`.
- **Reserved-word column identifiers (2026-09-25, user-approved)**: `PostgreSqlDialect`'s `generateInsert`/`generateUpdate`/`generateUpsert`/`generateInsertIfNotExists` and `ForeignKeyResolver.lookupParentId` never quoted column identifiers, so a real column named after a reserved word (e.g. `"order"` on `sample_profile_primary_sample`/`sample_profile_aliquot_sample`) broke with a Postgres syntax error. All column identifiers are now double-quoted everywhere they're interpolated into generated SQL — a no-op for every existing plain-lowercase column.
- **`UPSERT` requires a real unique/exclusion constraint on `naturalKeys` — importer2026 doesn't create one for you.** `ON CONFLICT (...)` only works if Postgres already has a matching constraint (e.g. `sample_type_quality_metadata`'s `uq_sample_type_quality`). If your target table's `naturalKeys` don't correspond to an actual constraint (as with `sample_profile_primary_sample`/`sample_profile_aliquot_sample`, which have none), use `operation: "INSERT"` instead — `naturalKeys` is still useful (FK caching / self-join row registration still works via `RETURNING id`), you just don't get idempotent re-runs without a prior `TRUNCATE`.
